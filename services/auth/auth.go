...
func HandleLoginPassword(app *container.Application, username, password string) (*models.LoggedUser, error) {
	...
	if err != nil {
		if errors.Is(err, models.RecordNotFound) {
			return nil, wrongCredentialsError{info: fmt.Sprintf("[AUTH] user \"%s\" not found", username)}
		}
	...
	if subtle.ConstantTimeCompare([]byte(passwordHash), []byte(user.PasswordHash)) == 0 {
		return nil, wrongCredentialsError{info: fmt.Sprintf("[AUTH] invalid password for \"%s\"", username)}
	}
